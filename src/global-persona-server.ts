...
import { ContextFormatter, ContextOptions } from './context-formatter.js';
...
export class GlobalPersonaServer {
  ...
  private setupRoutes(): void {
    ...
    // Context as markdown for Claude Code integration
  ...
  private async handleGetContext(req: Request, res: Response): Promise<void> {
    try {
      ...
      const memoriesResult = await this.memoryManager.retrieveMemories({
        projectHash,
      ...
      // Format as markdown
      const markdown = this.contextFormatter.formatAsMarkdown(
        projectHash,
